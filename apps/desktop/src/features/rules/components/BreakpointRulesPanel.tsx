...
export const BreakpointRulesPanel = forwardRef<RulesPanelHandle>(
  function BreakpointRulesPanel(_props, ref) {
    ...
    // P0-2 phase 2: the create-dialog draft lives in this panel's state, so a
    // tab switch unmounts it. Dirty = draft differs from an empty rule (the
    // comparator ignores ids, so the baseline can be created once).
    const emptyBaseline = useMemo(createEmptyBreakpointRule, []);
    const isDirty = useMemo(
      () => !isBreakpointRuleEqual(draft, emptyBaseline),
      [draft, emptyBaseline],
    );

    // Guards route navigation away AND losing the panel; both share one
    // confirmation dialog.
