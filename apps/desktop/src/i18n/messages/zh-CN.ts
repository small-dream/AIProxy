...
export const zhCNMessages: Messages = {
  ...
  certificatesPage: {
    ...
    mobile: {
      ...
      harmony: "鸿蒙",
      harmonySteps: [
        "确认你的鸿蒙 HarmonyOS NEXT 设备通过 USB 连接到本机并开启了 HDC 调试，同时与电脑处于同一 Wi-Fi 网络。",
        "在设备上打开“设置 > Wi-Fi”，点击已连接的网络，配置代理：主机填上方显示的本机 IP，端口填代理端口。（鸿蒙 HarmonyOS NEXT 不支持通过 hdc 设置全局代理，需要在 Wi-Fi 上手动配置。）",
        "点击上方的“通过 hdc 安装”把根证书推送到设备，然后在“设置 > 安全与隐私 > 加密与凭据 > 从存储设备安装”里继续安装刚才推送的证书文件。",
        "如果不想使用 hdc，也可以扫描上方二维码（或在设备浏览器打开链接）下载根 CA 证书，然后安装为“受信任的根证书（系统）”。",
        "鸿蒙 HarmonyOS NEXT 默认不信任用户安装的证书，对于做证书固定的应用，可能需要关闭目标应用的证书固定，或使用已 root 的设备把 CA 安装为系统证书。",
      ...
      hdcErrorTitle: "hdc 安装失败",
      hdcInstallAction: "通过 hdc 安装证书",
      hdcInstallBody:
        "通过 USB 连接一台鸿蒙 HarmonyOS NEXT 设备，并在开发者选项里开启 HDC 调试，然后点击按钮。AIProxy 会把根证书推送到设备，并尝试打开系统设置以便你手动完成安装。",
      hdcInstallHint:
        "这不是静默安装。鸿蒙 HarmonyOS NEXT 需要你在系统证书管理界面继续操作，选择从存储设备安装，并选中推送的证书文件。",
      hdcInstallRequirements: "要求本机 PATH 中可用 hdc（来自 HarmonyOS SDK / DevEco Studio），并且所选设备处于“Connected”状态。",
      hdcInstallUnavailable: "请先生成根证书，之后才能使用 hdc 安装。",
      hdcInstalling: "正在通过 hdc 安装...",
      hdcLoadingDevices: "正在检测 hdc 设备...",
      hdcManualInstallSteps: [
        "在设备上打开“设置 > 安全与隐私 > 加密与凭据”。",
        "点击“从存储设备安装”/“更多安全设置”，并选择证书文件。",
        "提示命名时可填写“AIProxy Root CA”，并确认安装为受信任的根证书（VPN 和应用）。",
        "在设备上重启浏览器或目标应用，使设置生效。",
      ],
      hdcManualProxyBody:
        "鸿蒙 HarmonyOS NEXT 没有可通过 hdc 设置全局 HTTP 代理的命令，因此需要在设备上手动配置 Wi-Fi 代理，地址见下方。",
