...
| 规则名称 | 便于识别 | — |
| 启用 | 控制是否生效 | 开启 |
| 优先级 | 数字越大越优先；多条命中取最高 | `100` |
| 来源 URL 模式 | 要匹配的请求 URL 模式 | — |
| 保留路径（Preserve Path） | 是否保留原请求的 path | 开启 |
| 保留 Query（Preserve Query） | 是否保留原请求的 query | 开启 |

来源 URL 模式按**子串匹配**（与 DNS 映射一致）：空值或 `*` 匹配全部，否则请求 URL 需包含该子串。例如 `example.com/assets/` 会匹配该路径下的请求。
...
1. 规则是否启用
2. 来源 URL 模式是否真的包含目标请求的 URL（子串匹配）
3. 目标路径是否存在、可读
4. 是否有更高优先级的规则先命中
