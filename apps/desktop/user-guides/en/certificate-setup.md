...
- **HarmonyOS NEXT device (USB)**: enable HDC debugging in developer options, then use **HarmonyOS Quick Action** to push the certificate to the device's `/data/local/tmp/` via `hdc` (requires `hdc` on PATH or `HDC_PATH` set, or DevEco Studio installed). After pushing, manually go to **Settings → Security & Privacy → Encryption & credentials → Install from storage** to finish the install. HarmonyOS NEXT has no ADB-equivalent global-proxy command, so set the system proxy manually in the device's Wi-Fi settings to point at your computer's IP:port. Being on the same Wi-Fi network is a prerequisite.
