...
> For first-time use, follow the in-app **Setup Wizard** (it auto-opens on first launch; afterwards you can reopen it from the setup checklist at the top of the **Sessions** page, or **Help → Setup Guide** on Windows/Linux). It walks you through: generate the root certificate → install & trust it → start the proxy → make sure SSL decryption is on → route traffic to AIProxy (system proxy or manual) → capture your first HTTPS request.
...
Open the **Mobile Setup** tab on the **Certificates** page. Mobile capture has a **preflight check**: you must first have "root certificate generated + proxy running + local LAN IP reachable" before the QR / ADB / hdc / Simulator panels unlock. On machines with multiple network adapters, use the local-IP selector so the QR code shows an address your phone can actually reach.

- **Android device (USB)**: enable USB debugging, then use the **Android Quick Actions** panel (requires `adb` on PATH or `ANDROID_HOME` set): **Install via ADB** pushes the certificate and opens Android's certificate settings — most devices still need you to confirm the install there; **Set Proxy via ADB** / **Clear Proxy via ADB** write or clear the phone's global HTTP proxy. When several devices are attached, pick the target in the panel's **ADB Device** dropdown.
- **Android emulator**: same ADB flow; emulators usually have root, so the cert can be installed as a system certificate.

After install + proxy are configured, use the **Verify phone traffic** card to check the chain end-to-end: click **Start checking**, open any site on the phone, and AIProxy reports how many new requests were captured. If nothing arrives before the timeout it lists likely causes (same Wi-Fi / AP isolation, proxy address mismatch, certificate not trusted, firewall).
