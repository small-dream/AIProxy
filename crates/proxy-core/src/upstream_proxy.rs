...
/// Match `host` against a bypass pattern list.
///
/// Supported pattern forms:
/// - `*` — bypass everything (an escape hatch that disables the proxy).
...
///   literal IP (we do not resolve hostnames to decide bypass, since that would
///   defeat the point of letting the upstream proxy do DNS).
