...
/// can be acquired from the synchronous `execute_hook` path (which the proxy
/// offloads to the blocking thread pool via `tokio::task::spawn_blocking`, see
/// proxy-core `rules/script.rs`) without pulling in an async semaphore
/// dependency.
        ...
        while *guard == 0 {
            ...
            let (next_guard, wait_result) = match lock_result {
                ...
                Err(poison) => {
                    ...
                    guard = g;
                    // The recovered pool may still be empty (the panicked thread
                    // held no permit). Issuing a guard here would over-commit
                    // slots, so fail open instead of saturating_sub'ing 0 -> 0.
                    if *guard == 0 {
                        return None;
                    }
                    break;
...
impl Drop for ScriptPermitGuard<'_> {
    fn drop(&mut self) {
        // Recover from a poisoned mutex instead of silently leaking the permit:
        // a panicking permit holder must not permanently shrink the pool.
        let mut guard = self
            .gate
            .available
            .lock()
...
fn execute_hook(
    ...
    match receiver.recv_timeout(SCRIPT_EXECUTION_TIMEOUT + Duration::from_millis(10)) {
        ...
        Ok(Err(error)) => runtime_failure_trace(
            rule,
            stage,
            // Classify by the error content (mirrors the timeout-eviction path
            // below): a genuine timeout surfaces as QuickJS "interrupted" from
            // the interrupt handler. Judging by wall-clock `start.elapsed()`
            // mislabels fast failures as TimedOut under load, because `start`
            // also covers the SCRIPT_GATE acquire wait.
...
fn run_script_in_thread(
    ...
    // Records that the interrupt handler aborted execution (cancel request or
    // wall-clock timeout). rquickjs collapses the interrupt exception into a
    // generic "Exception generated by QuickJS" (or a WouldBlock error for a
    // Promise chain), so without this tag the caller cannot tell an
    // interrupt-triggered failure (TimedOut) from an ordinary script error
    // (RuntimeError) by the error content alone.
    let interrupt_fired = Arc::new(AtomicBool::new(false));
    let handler_interrupt_fired = Arc::clone(&interrupt_fired);
    runtime.set_interrupt_handler(Some(Box::new(move || {
        let abort = cancel_flag.load(Ordering::Relaxed) || started_at.elapsed() >= timeout;
        if abort {
            handler_interrupt_fired.store(true, Ordering::Relaxed);
        }
        abort
    })));
    let context = Context::full(&runtime).map_err(|error| format!("create context: {error}"))?;

    let result = context.with(|ctx| -> Result<ScriptInvocationResult, String> {
    ...
    // Tag interrupt-triggered failures so the caller can classify them as
    // TimedOut by content (rquickjs collapses the original interrupt error).
    match result {
        Err(error) if interrupt_fired.load(Ordering::Relaxed) => {
            Err(format!("interrupted: {error}"))
        }
        other => other,
    }
}
...
mod tests {
    ...
    // The permit guard must return its slot even when the pool mutex is
    // poisoned (a prior thread panicked while holding the lock). The previous
    // `if let Ok(...) = lock()` silently skipped the increment, permanently
    // leaking one slot per poison event.
    #[test]
    fn permit_drop_recovers_from_poisoned_mutex() {
        let gate = ScriptConcurrencyGate::new();
        // Poison the mutex: panic while holding the lock.
        let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            let _guard = gate.available.lock().unwrap();
            panic!("intentional poison for test");
        }));
        assert!(gate.available.lock().is_err(), "mutex should be poisoned");

        let permit = gate
            .acquire(Instant::now() + Duration::from_secs(1))
            .expect("acquire recovers from poison");
        drop(permit);
        assert_eq!(
            *gate.available.lock().unwrap_or_else(|e| e.into_inner()),
            MAX_CONCURRENT_SCRIPT_THREADS,
            "dropped permit must be returned despite the poisoned mutex"
        );
    }

    // When the condvar wait wakes into a poisoned mutex whose pool is empty,
    // acquire must fail open (None) instead of issuing a permit on a
    // saturating_sub(0 -> 0) that over-commits slots.
    #[test]
    fn acquire_returns_none_when_poisoned_with_zero_slots() {
        let gate = Arc::new(ScriptConcurrencyGate {
        ...
        let poison_thread = thread::spawn(move || {
            let _ = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                let _guard = poisoned.available.lock().unwrap();
                panic!("intentional poison for test");
            }));
            // Keep notifying so a waiter parked on the condvar wakes and
            // reacquires the (now poisoned) mutex.
            for _ in 0..20 {
                poisoned.not_empty.notify_all();
                thread::sleep(Duration::from_millis(10));
            }
        });

        let result = gate.acquire(Instant::now() + Duration::from_secs(2));
        poison_thread.join().unwrap();
        assert!(
            result.is_none(),
            "acquire on an empty poisoned pool must fail open, not over-commit"
        );
    }

    fn test_rule(source: &str) -> CompiledScriptRule {
        CompiledScriptRule {
            rule: ScriptRule {
                id: "rule-1".to_string(),
            ...
            compiled_code: Arc::new(source.to_string()),
            source_map: None,
            compiled_match: None,
        }
    }

    fn test_payload() -> ScriptHookPayload {
        ScriptHookPayload {
    ...
    // A script that fails fast (syntax/eval error) must be classified as
    // RuntimeError even though `start.elapsed()` also covers the gate-acquire
    // wait — classification is driven by the error content, not wall clock.
    #[test]
    fn fast_script_failure_is_runtime_error_not_timed_out() {
        let rule = test_rule("this is not valid javascript");
        let result = execute_request_hook(&rule, test_payload());
        assert_eq!(result.trace.outcome, ScriptRunOutcome::RuntimeError);
    }

    // A hook that spins past the execution limit is interrupted by QuickJS and
    ...
    // Logging/extracting an unserializable payload (circular reference) must
    // not fail the whole hook: the bridge falls back to "[unserializable]"
    // for that entry's payload instead of letting JSON.stringify throw.
    #[test]
    fn circular_log_payload_falls_back_instead_of_failing_hook() {
        let rule = test_rule(
            "__aiproxyScriptExports.onRequest = function onRequest(ctx) { var o = {}; o.self = o; ctx.log.info('circular', o); ctx.extract('circ', o); };",
        );
        let result = execute_request_hook(&rule, test_payload());
        ...
        let log_entry = result
            ...
            .entries
            .iter()
            .find(|entry| entry.message.as_deref() == Some("circular"))
            .expect("log entry should exist");
        assert_eq!(log_entry.payload_json.as_deref(), Some("[unserializable]"));
