...
pub fn upsert_workspace(conn: &Connection, ws: &WorkspaceRow) -> Result<(), String> {
    conn.execute(
        "INSERT OR REPLACE INTO workspaces
            (id, name, proxy_port, ssl_enabled, system_proxy_enabled, storage_path, created_at, updated_at)
...
pub fn update_workspace(
    ...
    conn.execute(
        "UPDATE workspaces SET name=?1, proxy_port=?2, ssl_enabled=?3, updated_at=?4 WHERE id=?5",
        params![name, proxy_port, ssl_enabled as i32, updated_at, id],
    )
