...
- **批次 3b（性能 / DB / 规则）已完成 @ 2026-07-05**，修复 6 项（H2/M9/M10/M11/M14/M15），并经复审加固（4 项 follow-up）：
  ...
  - **M10** ✅ 每请求全量克隆改共享快照：`RewriteManager`/`ScriptManager` 存储 `Mutex<Arc<Vec<...>>>`，`compiled_rules()` 返回 `Arc<Vec<...>>`（refcount bump，无 per-rule 深拷贝），`set_rules`/`save_rule`/`delete_rule` 在锁内重建快照；`CompiledRewriteRule.compiled_match`/`CompiledScriptRule.{compiled_code,source_map,compiled_match}` 全部 `Arc` 包装（regex 不再重编译、~128KiB 编译代码串不再深拷贝）；`execute.rs` 的 `compiled_code.clone()` 变 `Arc::clone`；`active_*_rules_for_stage` 改按引用迭代快照。**复审加固**：`save_rule` 恢复原位替换语义（`iter_mut().find()` 原位替换而非 filter+push）——原 filter+push 把已存在规则编辑后移到列表末尾，UI 列表（keyed on `list_rules()` 顺序）会可见地重排。测试：`compiled_rules_snapshot_is_shared_until_mutation`（proxy-core + rule-engine 各一，断言 `Arc::ptr_eq`）+ `save_rule_preserves_position_of_existing_rule`。
  ...
  - **M15** ✅ **全部 DB-backed 命令移出 IPC 线程**（复审指出首轮只迁了 4 个命令，仍有 ~40 个同步命令持全局 DB 锁）。本轮全量迁移：`collections.rs`（list_api_collections/upsert/delete/move ×2/get_item/upsert_item/delete_item/save_session_to_collection）、`environments.rs`（7 个）、`rules.rs`（11 个：5 个 session-trace 查询 + set_breakpoint_rules/save_rewrite_rule/save_map_rule/save_script_rule/save_dns_mapping/delete_rule）、`throttling.rs`（save_throttle_profile/save_throttle_rule/delete_throttle_rule/set_active_throttle_profile）、`workspaces.rs`（create_workspace/update_workspace）全部改 `async` + `run_blocking_command`。仅读内存 manager 的命令（list_*_rules/profiles、list_workspaces/load_workspace、get_throttle_runtime_stats、resolve_breakpoint、get/inject_ws_message）保持同步。**manager 变更与 DB 写分离**：所有 save/delete 命令把 DB 写放闭包内、内存 manager 更新留 `.await?` 后。审计确认 commands/ 目录下已无同步 `.lock()` DB 访问点。
  - 门禁：`cargo build` ✅、`cargo fmt --check` ✅、`cargo clippy --workspace`（lib 目标）0 warning ✅（剩余 `--all-targets` test lint 全为预先存在）、`cargo test --workspace` 全绿 ✅（db 83、proxy-core 177、rule-engine 26、desktop 57，共 377）、`pnpm typecheck` ✅。
