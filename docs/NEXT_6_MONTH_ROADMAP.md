...
> 状态标注说明（2026-08-20）：M1–M3 的完成状态以仓库实现核验为准，不再使用不可追溯的完成日期。M3.5 已完成代码与自动化回归门槛，三端原生宿主机冒烟作为 `v0.1.20` 发布前的最后人工证据；在三端冒烟记录齐全前，M4 不进入主线 Beta。
...
**状态：✅ 代码与自动化回归门槛完成；三端发布冒烟待执行**（2026-08-20，经仓库实现核验）
...
已完成收口：Map Remote 目标 base path 合并与跨协议 runtime state 重建、失败结构化日志上下文、HTTP/2 上游连接 driver 异常驱逐、Map Remote/HAR/代理/TLS/节流相关 Rust 与前端回归。发布证据模板见 `scripts/release/m35-smoke-checklist.md`。
...
版本节奏以仓库当前发布状态为基线，不追溯重编号已经发布的版本。`v0.1.17`、`v0.1.18`、`v0.1.19` 已有对应双语发布摘要；M3.5 收口版本候选为 `v0.1.20`。版本号只有在发版提交、五处版本号同步和 `docs/releases/v<版本>.md` 准备完成后才视为确定计划。
...
| `0.1.19`（当前开发线） | 已完成 | M3.5 代码与自动化回归收口，等待 `v0.1.20` 发布证据 |

#### 后续候选（不绑定具体日期）

| 候选版本 | 计划窗口 | 核心含义 | 发布门槛 |
|---|---|---|---|
| `0.1.20` | 当前候选 | HTTP/2、Map Remote、TLS、上游代理、节流、HAR 和诊断维护版本 | 本地质量门禁通过；三端基础冒烟通过后创建稳定 tag |
