...
- 代理运行状态摘要
- 设置入口
- 发现新版本时，在窗口右侧显示“更新”图标入口；点击后打开更新详情对话框，版本号与更新说明在对话框内展示，继续沿用下载进度、安装与重启流程
...
- 仅在**疑似存在同名冲突**时出现：若所选目录下每个请求都落到不同文件，策略无意义，直接跳过对话框拉起目录选择器
- 冲突判定以 `host + URL 路径（去 query）` 为键，是**启发式预检**：能抓住「同路径不同 query」这类常见冲突，但后端还会做 dot-segment 归并、百分号解码、按 MIME 补扩展名、目录 URL 落 `index.<ext>` 等归一化，可能撞出预检没预见的冲突。漏报是安全方向的取舍——此时隐式走 `keepAll`（加序号全保留，不丢数据），但用户失去了选 `latestOnly` 的机会。有意不在此复刻后端清洗逻辑，避免两套实现漂移
- 由目录 / host 右键的 `Save All Files...` 打开，确认后才由后端拉起系统目录选择器
- 顶部以 `Alert` 呈现「即将保存 N 个请求」，N 已排除 WebSocket 会话
- 单选组决定同一文件被多次抓到时的取舍：默认「只保留最后一次请求」（重复导出到同一目录时**覆盖**该路径上的旧导出文件），另一项为「保存所有请求」（重名加序号，`create_new` 原子创建，既有符号链接只会计为占用、绝不穿透）
- 底部以 caption 说明目录层级还原规则与跳过策略，避免用户对落盘结果产生误解
- 用户在系统目录选择器中取消时对话框保持打开，不丢失已选策略
- 保存结果通过页面级 Snackbar 汇总，失败与跳过分开表述：「已保存 N 个」（全部成功）、「已保存 N 个，跳过 M 个」（有 WS/空响应）、「已保存 N 个，F 个写入失败」（有失败，绝不把失败说成跳过）、「没有文件成功保存到 …」（全部失败，区别于「没有可保存的文件」）；存在被截断上限裁剪过的导出文件时额外追加不完整提示
