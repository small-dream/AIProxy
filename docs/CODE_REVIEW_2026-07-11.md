...
- **状态：已修复 @ batch 8（2026-07-11），batch 10 加固（2026-07-12）** — 采用专用 helper（避免改宽 `update_workspace` 签名）：`workspaces::set_workspace_system_proxy_enabled(conn, id, enabled)` 单列 UPDATE。在两个 toggle 路径接入：`enable_system_proxy_impl` 与 `disable_system_proxy_impl` 在内存态切换后用 `run_blocking_command` + `lock_db_for_ipc` 写回 DB。`workspace_id` 取自 `status.active_workspace_id`。测试：`m9_set_workspace_system_proxy_enabled_round_trips`。
  - **batch 10 加固（degraded state 替代 best-effort）**：原实现持久化失败只 `warn` 后照常返回 enabled/disabled，DB 行仍 stale，M9 跨重启漂移仍可复现。现改为：OS proxy 已 apply/restore 后若 DB 写失败，**设置 `system_proxy_recovery_warning`** 让 UI 可提示用户重试（而非回滚已 apply 的 OS proxy——回滚比 drift 本身更具破坏性）。返回的 `BootstrapStatus` 携带 warning 字段，前端据此显示降级态。enable/disable 两路径对称处理。
...
- **状态：已修复 @ batch 8（2026-07-11），batch 10 加固（2026-07-12）** — `root_cert_exists` 从"只查 `exists()`"升级为校验完整 PEM block。**batch 10 加固**：原 substring-only 检查（`contains("BEGIN CERTIFICATE")`）对截断 PEM（有 BEGIN fence 但 base64 被截、无 END fence）返 true，导致 `generate_root_certificate_impl` 跳过重建后在 `load_from_pem` 失败。现改用 `pem_block_is_complete(pem, label)`：要求匹配的 BEGIN/END fence 且中间 base64 体可 `STANDARD.decode` 解码（剥离 PEM header 行）。key 支持 `PRIVATE KEY`/`EC PRIVATE KEY`/`RSA PRIVATE KEY`/`ENCRYPTED PRIVATE KEY` 四种 label。测试：`m14_root_cert_exists_false_for_empty_files`、`m14_root_cert_exists_false_for_missing_pem_fence`、`m14_root_cert_exists_false_for_truncated_pem`（新增，覆盖 torn-write 形态）、`m14_root_cert_exists_true_for_valid_pair`、`m14_pem_block_is_complete_rejects_truncated_body`（helper 单元测试：合法 block true、无 END false、非法 base64 长度 false、无 BEGIN false、label 不匹配 false）。
