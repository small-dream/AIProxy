...
- **建议拆分**：
  ...
  - **P2.5：明文 HTTP 解析路径 hyper 化**：将当前 `read_proxy_request_from_stream` 的手写 HTTP/1.1 解析逐步收敛到 hyper request/body 模型，统一明文 HTTP 与 HTTPS MITM 的请求构建、body 读取、超时、大小限制和 raw message 生成。短期 chunked body 已采用就地解码；长期目标是减少手写协议解析边界风险，而不是把该重构混入单点 bugfix。
